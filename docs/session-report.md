...
- 10:3x — PR #21 (OWASP) merged. **Revisão dos relatórios** para o processo seletivo: narrativa de protagonismo do candidato + painel de ferramentas (PR #22). **Correção de identidade**: usuário é Gustavo (masculino) — todas as ocorrências corrigidas (commit `2267331`); **lição registrada**: push direto em `main` burlou a branch protection (erro do agente) → `enforce_admins: true` para nunca mais.
- 10:4x — **Colaboradores convidados** (autorizado pelo usuário): `ygorbalves` e `MateusNavarroR` (permissão write, convites pendentes de aceite); issue #11 fechada — **T1–T10 concluídos**.